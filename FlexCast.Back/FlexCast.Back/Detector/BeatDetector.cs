...
namespace BeatDetectorCSharp
{
    public class BeatDetector
    {
        ...
        public void LoadSong(int sSize, string audioString)
        {
            ...
            areWePlaying = true;
            specFlux = 0.0f;
            timeBetween = TimeSpan.FromMilliseconds(0);
            currentSeconds = 0;
            currentMillis = 0;
            currentMinutes = 0;
            median = 0.0f;
            ...
            FMODErrorCheck(system.playSound(FMOD.CHANNELINDEX.FREE, audio, false, ref songChannel1));
        ...
        public void update()
        {
            ...
            //Beat detected
            if (specFlux > beatThreshold && (position - timeBetween).TotalMilliseconds > 350)
            {
                ...
                timeBetween = getPosition();

                TimeStamp t = new TimeStamp(position.Minutes, position.Seconds, position.Milliseconds, specFlux);
            ...
            else if ((getPosition() - timeBetween) > TimeSpan.FromSeconds(5))
            {
        ...
        {
            uint var = default;
            songChannel1.getPosition(ref var, TIMEUNIT.MS);
            return TimeSpan.FromMilliseconds(var);
        }
        public void setPosition(TimeSpan position)
        {
            songChannel1.setPosition((uint)position.TotalMilliseconds, TIMEUNIT.MS);
        }
