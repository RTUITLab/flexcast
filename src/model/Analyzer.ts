export class Analyzer {
  ...
  public Analyze(seconds: number): Promise<Beats> {
    ...
    const headPromise = this.AnalyzeInternal(headPart, headAnalyzer, 0);
    const tailPromise = this.AnalyzeInternal(
      tailPart,
      tailAnalyzer,
      this.data.duration - 10
    );
    return Promise.all([headPromise, tailPromise]).then(
      (args) => new Beats(args[0], args[1])
    );
  }
  ...
  private calculateFluxAndSmoothing(currentSpectrum: Float32Array): number {
    ...
    for (let i = 0; i < this.analyser.frequencyBinCount; i++) {
      if (this.spectrumFluxes.length > 1)
        this.spectrumFluxes.splice(
          this.spectrumFluxes.length - 1,
          0,
          this.specFlux
        );
      else
        this.spectrumFluxes.splice(
          this.spectrumFluxes.length,
          0,
          this.specFlux
        );
    ...
    if (smoothMedian > 1) this.thresholdSmoother = 0.8;
    if (smoothMedian > 2 && smoothMedian < 4) this.thresholdSmoother = 1.0;
    if (smoothMedian > 4 && smoothMedian < 6) this.thresholdSmoother = 2.2;
    if (smoothMedian > 6) this.thresholdSmoother = 2.4;

    return this.thresholdSmoother + median;
  }

  public update(): void {
    if (Date.now() > this.end) this.resolve(this.beats);
    ...
    if (this.specFlux > beatThreshold && Date.now() - this.timeBetween > 350) {
      if (this.smootherValues.length > 1)
        this.smootherValues.splice(
          this.smootherValues.length - 1,
          0,
          this.specFlux
        );
      else
        this.smootherValues.splice(
          this.smootherValues.length,
          0,
          this.specFlux
        );
      ...
      if (this.thresholdSmoother > 0.4) this.thresholdSmoother -= 0.4;
