...
export class SampleMerger {
    public static MergeSamples(samples: Sample[]) {
        ...
        for (let index = 1; index < samples.length; index++) {
            ...
            const matchLevel = 5;
            const bestMatch = SampleMerger.findSimilar(matchLevel, prev.source.beats, element.source.beats);
            console.log(bestMatch);
            const firstMiddle = (prev.source.beats.tail[bestMatch.tail] + prev.source.beats.tail[bestMatch.tail + matchLevel - 1]) / 2;
            const secondMiddle = (element.source.beats.head[bestMatch.head] + element.source.beats.head[bestMatch.head + matchLevel - 1]) / 2;
            element.offset = prev.offset + firstMiddle - secondMiddle;
        }
    }

    private static findSimilar(count: number, beats1: Beats, beats2: Beats): BestBeats {
